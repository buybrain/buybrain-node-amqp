...
function newConnection(options) {
    ...
    function decorateChannel(chan) {
        const originalPublish = chan.publish;

        chan.publish = function (exchange, routingKey, content, options) {
            return new Promise(accept => {
                if (originalPublish.call(chan, exchange, routingKey, content, options)) {
                    accept();
                } else {
                    chan.once('drain', accept);
                }
            });
        }
    }
